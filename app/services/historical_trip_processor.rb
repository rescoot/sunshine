class HistoricalTripProcessor
  ...
  def initialize(scooter_ids: nil)
    @scooter_ids = Array(scooter_ids).map(&:to_i).reject(&:zero?)
    @stats = {
  ...
  def process
    ...
    # Filter scooters if IDs are provided
    scooters = if @scooter_ids.any?
                 puts "Processing only scooters with IDs: #{@scooter_ids.join(', ')}"
                 Scooter.where(id: @scooter_ids)
    else
                 puts "Processing all scooters"
                 Scooter.all
    end

    # Process each scooter
    scooters.find_each do |scooter|
      process_scooter(scooter)
    end

    # Print summary
    print_summary

    # Return stats for potential use in admin interface
    @stats
  end

  def process_scooter(scooter)
    puts "Processing scooter: #{scooter.name} (#{scooter.vin})"
    @stats[:scooters_processed] += 1

    # Find the last completed trip for this scooter
    last_trip = scooter.trips.completed.order(ended_at: :desc).first

    # Get telemetry for this scooter, ordered chronologically
    telemetry_query = scooter.telemetries

    # If there's a last trip, only process telemetry after it ended
    if last_trip
      puts "  Found last trip ended at #{last_trip.ended_at}"
      telemetry_query = telemetry_query.where("created_at > ?", last_trip.ended_at)
    end

    telemetries = telemetry_query.order(created_at: :asc)
    ...
    telemetries.each_with_index do |telemetry, index|
      ...
      if previous_telemetry
        ...
        if TripDetectionService.would_end_trip?(telemetry, previous_telemetry) && current_trip_start
          ...
          # Create a trip
          create_trip(scooter, current_trip_start, telemetry)
  ...
  def create_trip(scooter, start_telemetry, end_telemetry)
